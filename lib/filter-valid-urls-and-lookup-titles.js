...
function geneneratePromiseForTitleLookup(url, template, delayMs) {
  function setReplacementTimeout(resolve) {
    function takeTitleAndGenerateReplacementEntry() {
    ...
    setTimeout(takeTitleAndGenerateReplacementEntry, delayMs);
  }
...
function shouldGeneratePromise(currentUrlIndex, currentUrl, markdown) {
...
module.exports = function (urls, markdown, hoganTemplate) {
  ...
  function iterateOverUrls(currentUrl) {
    ...
    while (markdown.indexOf(currentUrl, urlIndexTracker) !== -1) {
      ...
      if (!shouldGeneratePromise(currentUrlIndex, currentUrl, markdown)) {
        continue;
      }
